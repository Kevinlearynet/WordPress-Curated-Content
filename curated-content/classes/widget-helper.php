...
if ( !class_exists( 'WPH_Widget' ) ) {
	class WPH_Widget extends WP_Widget
	{
		...
		function create_field_radio( $key, $out = "" )
		{
			...
			$out .= '<div class="radio-container">';

			$checked = isset( $key['value'] ) ? $key['value'] : $key['std'];

			foreach ( $key['fields'] as $field => $option ) 
			{
				$out .= '<input type="radio" id="' . esc_attr( $key['_id'] ) . '" name="' . esc_attr( $key['_name'] ) . '" value="' . esc_attr__( $option['value'] ) . '" ';

				if ( isset( $key['class'] ) )
					$out .= 'class="' . esc_attr( $key['class'] ) . '" ';
					
				// Checked
				if ( esc_attr( $checked ) == $option['value'] )
					$out .= ' checked';
	
				$out .= ' /> <span>' . $option['name'] . '</span><br />';
			}

			$out .= '</div><!--// end .radio-container -->';
